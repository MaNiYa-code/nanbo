...
const FIXED_COST_PRESETS = [
  { label: '実家住み',    icon: '🏠', value: 50000,  desc: '食費・交通費など' },
  { label: '一人暮らし', icon: '🏢', value: 150000, desc: '家賃込みの目安' },
  { label: '都市部',     icon: '🏙', value: 200000, desc: '東京・大阪など' },
];
  ...
  return (
    <div>
      <div className="flex items-baseline justify-between mb-1.5">
        ...
        <span className="text-sm font-black" style={{ color: '#70191D' }}>{formatCurrency(value)}</span>
      </div>
...
export function NanboSettings({ settings, onUpdateSettings, onClose }) {
  const [local, setLocal] = useState({
    ...
    monthlyFixedCost:  settings.monthlyFixedCost,
    monthlyInvestment: settings.monthlyInvestment,
  });

  const handleChange = (e) => {
    const val = Math.max(0, Number(e.target.value));
    setLocal(prev => ({ ...prev, [e.target.name]: val }));
  };
  ...
  const free = local.monthlyIncome - local.monthlyFixedCost - local.monthlyInvestment;
        ...
        <div className="px-5 pb-8 overflow-y-auto" style={{ maxHeight: '85vh' }}>
          ...
          <div className="space-y-5">
            ...
            <div className="rounded-2xl p-4" style={{ background: '#FFF8F0', border: '1.5px solid #EDD5C0' }}>
              ...
              <div className="flex gap-2 mt-3">
                {INVESTMENT_PRESETS.map(p => (
                  <button
                    key={p.value}
                    onClick={() => setLocal(prev => ({ ...prev, monthlyInvestment: p.value }))}
                    className="flex-1 py-2 rounded-xl text-xs font-black transition-all active:scale-95"
                    style={local.monthlyInvestment === p.value
                      ? { background: '#970A12', color: '#FECD6D' }
                      : { background: '#EDD5C0', color: '#70191D' }
                    }
                  >
            ...
            <div className="rounded-2xl p-4"
              style={{
                background: isNegative ? '#FFF0EE' : '#EEF3FA',
                border: `1.5px solid ${isNegative ? '#FFCDC8' : '#5686BB44'}`,
              }}>
              <p className="text-xs font-black mb-2" style={{ color: isNegative ? '#970A12' : '#1A3A5C' }}>
                {isNegative ? '⚠️ プレビュー：赤字になっています' : '💡 プレビュー：自由に使えるお金'}
              </p>
              <div className="space-y-1 text-xs" style={{ color: '#8B5840' }}>
                <div className="flex justify-between">
                  <span>月収</span>
                  <span className="font-bold">{formatCurrency(local.monthlyIncome)}</span>
                </div>
                <div className="flex justify-between">
                  <span>− 固定費</span>
                  <span className="font-bold">{formatCurrency(local.monthlyFixedCost)}</span>
                </div>
                <div className="flex justify-between">
                  <span>− 積立</span>
                  <span className="font-bold">{formatCurrency(local.monthlyInvestment)}</span>
                </div>
